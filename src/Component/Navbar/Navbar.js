            ...
            <div className='flex justify-between xl:justify-center items-center md:items-center relative top-0 left-0'>
                ...
                <div>
                    ...
                    {open && <div className={`fixed overflow-scroll top-0 left-0 right-0 bottom-0  ${open ? 'w-[100vw] h-[100vh] opacity-100' : 'w-[0vw] h-[0vh] opacity-0'} bg-[#181A23] duration-10000 z-50`}>
                        ...
                        <ul>
                            {navItem.map((item) => <li key={item.id} className='list-none my-[12px] text-center'><a className='uppercase text-[22px] text-[#D9D9D9] duration-500 hover:text-[#ffffff] tracking-[1.5px] leading-[40px] hover:bg-[#fff] hover:py-2 hover:px-1 rounded-md hover:text-[#000] hover:font-[600]' href='#'>{item.name}</a></li>)}
                        </ul>
                    </div>}
                </div>
            </div>
